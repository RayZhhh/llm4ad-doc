...
class ProgramsDatabase:
    ...
    @property
    def best_target_func(self):
        best_target_func = None
        best_target_score = float('-inf')

        for island in self._islands:
            for k, v in island.clusters.items():
                for func in v.programs:
                    if func.target_score is not None and func.target_score > best_target_score:
                        best_target_score = func.target_score
                        best_target_func = func

        return best_target_func
    ...
    def best_func(self):
        ...
        for island in self._islands:
            for k, v in island.clusters.items():
                for func in v.programs:
                    if func.score is not None and func.score > best_score:
                        best_score = func.score
                        best_func = func
...
class Island:
    ...
    def get_prompt(self) -> tuple[str, int]:
        ...
        if max_abs_score > 1:
            cluster_scores = cluster_scores.astype(float) / (max_abs_score + 1e-6)
        # ------------------------------------------------------------------------------
...
class Cluster:
    ...
    def sample_program(self) -> Function:
        """Samples a program, giving higher probability to:
            - Shorter programs.
            - Programs with better target scores.
        """
